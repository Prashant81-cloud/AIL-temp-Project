...
export default function FAQDesktop() {
 ...
 return (
    <div className="w-full min-h-screen flex justify-center items-start bg-[#FAF4EC] px-4 sm:px-6 lg:px-8 py-10">
      <div className="w-full h-full max-w-6xl flex flex-col md:flex-row gap-8 md:gap-12 xl:gap-16 items-start">
        ...
        <aside className="w-full md:w-1/3">
          ...
          <div className="font-bold leading-tight mb-6 archivo-black">
            ...
            <span className="block mt-2 text-[#5C8AC9] text-xl sm:text-2xl md:text-[26px] lg:text-[31px]">
              Asked Questions
            </span>
          </div>

          {/* Category Tabs */}
          <div className="flex md:flex-col gap-3 md:gap-4 lg:gap-5 mt-4 overflow-x-auto md:overflow-visible pb-1 justify-start items-start">
            {categories.map((cat, idx) => (
              <button
                ...
                className={`whitespace-nowrap text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl font-semibold  
                  
                  md:pl-3 pb-1 md:pb-0
                  ${
                    activeCategory === idx
                      ? "text-black "
                      : "text-gray-500 "
        ...
        {/* RIGHT CONTENT */}
        <main className="w-full md:w-2/3 flex flex-col md:gap-4 lg:gap-6 h-full  lg:mt-30 md:mt-25">
          {categories[activeCategory].questions.map((item, index) => (
            <div
              key={index}
              className="border-b border-gray-300 pb-4 last:pb-0"
            >
              <button
                onClick={() => handleToggle(index)}
                className="w-full flex justify-between items-center gap-3 text-base sm:text-lg md:text-lg lg:text-xl xl:text-2xl font-semibold text-left"
              >
                <span>{item.question}</span>
                {openIndex === index ? (
                  <FiMinus className="text-lg sm:text-xl text-gray-600 flex-shrink-0" />
                ) : (
                  <FiPlus className="text-lg sm:text-xl text-gray-600 flex-shrink-0" />
                )}
              </button>
